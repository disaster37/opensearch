...
// Sorter is the common interface implemented by all sorting strategies.
// Any type that implements Source() (any, error) can serve as a Sorter.
// Built-in implementations include [FieldSort], [ScoreSort], [GeoDistanceSort],
// [ScriptSort], and [SortByDoc].
//
...
// SortInfo contains information about sorting a field. It implements the
// [Sorter] interface and can be used directly with [SearchSource.SortWithInfo].
...
// ScoreSort sorts search results by their relevancy score (_score).
// By default, results are sorted in descending order.
//
// Typical usage:
//
...
// FieldSort sorts search results by a given field. It supports specifying
// the sort order, handling missing values, and sorting on nested fields.
//
// Typical usage:
//
...
// GeoDistanceSort sorts search results by the distance between a geo field
// and one or more reference points. It supports specifying the distance unit,
// computation type (arc or plane), and handling of unmapped fields.
//
...
// ScriptSort sorts search results by a custom script. The script must return
// values that can be compared, and the type must be specified as either
// "string" or "number".
//
// See https://www.opensearch.co/guide/en/opensearchsearch/reference/7.0/modules-scripting.html#modules-scripting
