...
// Aggregation is the interface implemented by all OpenSearch aggregation
// builders. It produces a JSON-serializable fragment describing the
// aggregation that is sent as part of the search request body.
//
// Aggregations are OpenSearch's analytic framework (the successor to facets):
// they compute metrics, group documents into buckets, and perform pipeline
// calculations over a result set. For full details see the OpenSearch
// aggregations reference.
//
// All concrete aggregation types (TermsAggregation, DateHistogramAggregation,
// MinAggregation, etc.) implement this interface.
...
// Aggregations is a map of aggregation name to raw JSON, representing the
// aggregation results returned in a search response. It provides typed
// accessor methods (e.g. Terms, Min, DateHistogram) that unmarshal the
// raw JSON for a named aggregation into the appropriate response struct.
...
// Value maybe one of map[string]any/[]any/string/bool/json.Number
