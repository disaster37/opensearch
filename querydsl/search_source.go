...
// SearchSource is the programmatic builder for an OpenSearch _search request body.
// It produces the JSON payload sent to /_search and supports all features
// of the REST API: query, filter, aggregations, sorting, highlighting, collapse,
// suggesters, paging, and more.
//
// Typical usage:
//
...
type SearchSource struct {
	...
	trackTotalHits           any                    // track_total_hits
	searchAfterSortValues    []any                  // search_after
...
func (s *SearchSource) Source() (any, error) {
	...
	if s.fetchSourceContext != nil {
		source["_source"], _ = s.fetchSourceContext.Source()
	}
	...
	if len(s.docvalueFields) > 0 {
		src, _ := s.docvalueFields.Source()
		source["docvalue_fields"] = src
	}
	if len(s.fields) > 0 {
		src, _ := s.fields.Source()
		source["fields"] = src
	}
	...
	if len(s.indexBoosts) > 0 {
		src, _ := s.indexBoosts.Source()
		source["indices_boost"] = src
	}
	if len(s.aggregations) > 0 {
		aggsMap := make(map[string]any)
	...
	if len(s.suggesters) > 0 {
		suggesters := make(map[string]any)
	...
	if s.pointInTime != nil {
		source["pit"], _ = s.pointInTime.Source()
	}
