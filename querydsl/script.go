...
// and then execute a script. It supports inline scripts (type "inline"/"source"),
// stored scripts (type "id"), and custom scripting languages such as Painless.
//
...
func (s *Script) Source() (any, error) {
	...
	if s.typ == "" || s.typ == "inline" {
		source["source"] = s.rawScriptSource(s.script)
	} else {
		source["id"] = s.script
	}
	if s.lang != "" {
...
func (s *Script) rawScriptSource(script string) any {
...
// ScriptField defines a response field whose value is computed by a [Script]
// at search time. Script fields are useful for returning derived or computed
// values without modifying the stored data.
