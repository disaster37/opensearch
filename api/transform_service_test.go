...
func newTransformTestServer() *httptest.Server {
	...
	mux.HandleFunc("/_plugins/_transform/test_job/_explain", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"test_job":{"metadata_id":"meta1","transform_id":"test_job","last_updated_at":1000,"status":"started","failure_reason":"","stats":{"pages_processed":10,"documents_processed":100,"documents_indexed":50,"index_time_in_millis":500,"search_time_in_millis":200}}}`))
	})
	...
	mux.HandleFunc("/_plugins/_transform/test_job", func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.Method {
		case http.MethodGet:
			w.Write([]byte(`{"_id":"test","_version":1,"_seq_no":0,"_primary_term":1,"transform":{"source_index":"source","target_index":"target"}}`))
		case http.MethodPut:
			w.Write([]byte(`{"_id":"test","_version":2,"_seq_no":1,"_primary_term":1,"transform":{"source_index":"source","target_index":"target"}}`))
